...
<% for( i=0;i<listday.size();i++){ %>	
	<% out.print(", \""+listday.get(i) + "\"");%>: [{
		...
		dataPoints: <%
		PreparedStatement ps = con.prepareStatement("select  prces_dte, sum((actual*point)) as throughput from transaction a, task b ,users c where a.task_id=b.task_id and a.user_id=c.user_id and to_char(prces_dte,'YYYY')=? and to_char(prces_dte,'MM')=?  group by prces_dte order by prces_dte asc");
